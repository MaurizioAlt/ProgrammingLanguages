...
test1Problem1 n = reverse( take 10 (reverse( show (sum(go 1 n)) )) )
  where
  go s n 
    | s > n = []
    | otherwise = (s^s) : go (s+1) n 

---------------------------

primeList n = go (n*3) 1 2
  where
  go n c p
    | c > n = []
    | isPrime p = p : go n (c+1) (p+1)
...
dropEvery _ [] = []
dropEvery n xs = take (n-2) xs ++ dropEvery n (drop n xs)

dropThird n = dropEvery 3 (primeList n)


test1Problem2 n = zipWith (*) [0..] (dropThird n)

----------------------------

test1Problem3 2 = go 2 2
  where
  go n s
    | (length(primeFactors s) == n) && (length(primeFactors (s-1)) == n) = (s-1)
    | otherwise = go n (s+1)


test1Problem3 3 = go 3 14
  where
  go n s
    | (length(primeFactors s) == n) && (length(primeFactors (s-1)) == n) && (length(primeFactors (s-2)) == n) = (s-2)
    | otherwise = go n (s+1)


test1Problem3 4 = go 4 134046
  where
  go n s
    | (length(primeFactors s) == n) && (length(primeFactors (s-1)) == n) && (length(primeFactors (s-2)) == n) && (length(primeFactors (s-3)) == n) = (s-3)
    | otherwise = go n (s+1)
